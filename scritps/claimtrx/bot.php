...
//            "x-requested-with: XMLHttpRequest",
    ...
    global $l;
    $r = explode('.gif"', explode('src="https://claimtrx.com/assets/images/captcha/', $r)[1])[0];
    $r = Run("https://claimtrx.com/assets/images/captcha/$r.gif", header0());
...
function faucet(){
  global $l;
  $r = Run("https://claimtrx.com/faucet",header0());
            if(preg_match('/Just a moment.../', $r)){
        cf_bypass("https://claimtrx.com/faucet");
        rewardBox("CF",["msg" => "CF-BYPASSED"]);
        return faucet();
    }
  $bal = explode('</h2>', explode('<h2 class="mb-3 font-18">', $r)[3])[0];  
    if($bal == ""){unlinkData(host,'cookie');SaveData(host,'cookie');return faucet();}
    rewardBox("Dashboard",["Balance" => $bal . " TRX"]);
      while(true){    
  $r = Run("https://claimtrx.com/faucet",header0());
                      if(preg_match('/Just a moment.../', $r)){
        cf_bypass("https://claimtrx.com/faucet");
  ...
  $r = Run("https://claimtrx.com/faucet/verify",headers('claimtrx.com'),$req);
  $r = Run("https://claimtrx.com/faucet",header0());
            if(preg_match('/Just a moment.../', $r)){
        cf_bypass("https://claimtrx.com/faucet");
        rewardBox("CF",["msg" => "CF-BYPASSED"]);
  ...
  $bal = explode('</h2>', explode('<h2 class="mb-3 font-18">', $r)[3])[0];  
    ...
    if($msg){
        ...
        rewardBox("Faucet",["Reward" => $msg,"Text Code" => $cap,"Balance" => $bal . " TRX"]);
        } 
    }        
}
